---
title: pushinbr/pam-http-testing
description: Test pushinbr/pam-http applications in memory with fluent response assertions.
---
...
:::note[Migrating from the old name]
`pushinbr/pam-testing` 2.x is an abandoned compatibility package that installs
`pushinbr/pam-http-testing` and preserves the legacy `Pam\Testing` aliases.
New code should import `Pam\Http\Testing` and require the permanent package name.
:::
...
:::note[License]
`pushinbr/pam-http-testing` uses the Apache License 2.0.
:::
