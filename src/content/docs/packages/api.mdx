...
| Type | Public surface |
| --- | --- |
| `App` | `get()`, `post()`, `put()`, `patch()`, `delete()`, `route()`, `middleware()`, `handler()`, `provider()`, `onError()`, `listen()`, `handle()` |
...
| `Route` | Readonly method, path, handler closure, compiled pattern and parameter names |
