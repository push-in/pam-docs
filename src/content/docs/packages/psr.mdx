...
`pushinbr/pam-psr` 2.x is an abandoned compatibility package that installs
`pushinbr/pam-http-psr`. Existing applications keep working, but new manifests
must use the permanent HTTP-family name shown above.
:::
...
:::note[License]
`pushinbr/pam-http-psr` uses the Apache License 2.0.
:::
