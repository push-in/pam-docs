...
The official PAM release does not require a system PHP CLI. Composer executes
inside the same PHP 8.5 Embed environment used by the application by default,
so platform requirements are checked against the relevant runtime.
...
| PSR-7, PSR-15, PSR-17 | Available through `pushinbr/pam-http-psr` |
