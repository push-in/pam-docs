...
PAM Mobile UI's `Grid` and `GridItem` utilities compile into PAM Native's
...
Use the [responsive layout guide](../../native/responsive-layout/) for the
twelve-column low-level API, responsive spans, rich virtualized grids, and
integer `v-for`.
