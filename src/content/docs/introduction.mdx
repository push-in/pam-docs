...
The server runtime is currently experimental at `0.1.x`. PAM Native is Android-only and uses protocol version `1`. PAM Mobile UI exposes a broad but individually graded component surface. PAM Desktop targets controlled prototypes and product exploration while Servo and multi-platform distribution continue to mature.

The [project status](/project/status/) page is the shortest accurate overview of what you can use today.
