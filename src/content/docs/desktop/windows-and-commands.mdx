...
Windows declared in the application are reusable. When the user clicks the native close control on a secondary window, PAM blurs and hides its WebView instead of destroying the configured window. A later visible/focus effect can open the same window again without rebuilding the Servo pipeline.

Closing the main window still exits the application. Explicit close effects may destroy a secondary window; native teardown releases the Servo WebView first, then its rendering context, and finally the Winit window so callbacks never target an already-destroyed surface.
...
Protocol 4 serializes commands through one PHP worker mutex. If a deadline expires, the caller aborts, the worker crashes, emits invalid output, or exceeds the message limit, PAM kills and reaps the worker and prepares a fresh generation.
