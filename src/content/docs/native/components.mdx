...
PAM Native 0.5.52 also expands relative CSS imports at compile time:
...
Imports may be nested and remain scoped to the importing component. They must
be relative `.css` files inside the nearest Composer project; network imports,
cycles, traversal and oversized graphs fail compilation. Imported contents
participate in component cache invalidation, and local rules after the import
win through normal cascade order.
...
position edges, flex growth/shrink/direction, gap and alignment, padding and
margin shorthands, borders and radii, backgrounds, opacity, elevation,
overflow, and text/font properties. Plain numbers and `px`, `dp`, or `pt`
represent PAM logical points.
...
preserved. The `--check` form is intended for CI.
