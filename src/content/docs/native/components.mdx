...
alignment, and case inherit through native layout containers. Logical font
families stay intact during inheritance, so a descendant that changes only its
weight can select the corresponding packaged `@font-face`.
...
Auto-width text keeps a conservative intrinsic frame to avoid clipping custom
font glyphs. The platform renderer aligns visible glyphs from the relevant
flex axis: `align-items`/`align-self` in columns and `justify-content` in rows.
This keeps centered text-and-icon controls optically centered while preserving
safe text measurement. Explicitly sized or growing text keeps normal start
alignment unless `text-align` is authored. Use the familiar CSS values
`left`, `center`, and `right`, or the logical aliases `start` and `end`.
