...
Auto-width text is measured from calibrated native sans-serif glyph advances
with a sub-pixel wrap tolerance. The platform renderer aligns visible glyphs
from the relevant flex axis: `align-items`/`align-self` in columns and
`justify-content` in rows. Since PAM Native `0.5.83`, this removes the inflated
safety width that could shift centered text-and-icon controls while preserving
custom-font overhang and stable wrapping. Explicitly sized or growing text
keeps normal start alignment unless `text-align` is authored. Use the familiar
CSS values `left`, `center`, and `right`, or the logical aliases `start` and
`end`.
