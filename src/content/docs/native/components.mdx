...
<style scoped>
    :root {
        --font-bold: asset://assets/fonts/Brand-Bold.ttf;
        --muted: #5C5C55;
    }
...
component-local `:root` variables are accepted. Descendant selectors, nested
rules, media queries, and properties without a native protocol contract fail
...
`v-*` directives to `p-*`. The `--check` form is intended for CI.
